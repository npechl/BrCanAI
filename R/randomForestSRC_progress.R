...
df_r$pnumber = NULL
df_r$death = NULL
df_r$survival_time = NULL


# remove not available records --------------------------------

df_r = df_r[which(!is.na(progress) & !is.na(progression_time))]
...
df_filtered = df_filtered[, c(x, "progress", "progression_time"), with = FALSE]

# build model --------------------

obj <- rfsrc(
    Surv(progression_time, progress) ~ ., df_filtered,
    ntree = 500, 
    nodesize = 5, 
    nsplit = 50, 
    importance = TRUE
...
pdf(file = "output/Progression predictors/Rplot.pdf", width = 12, height = 6)
...
get.cindex(obj$yvar$progression_time, obj$yvar$progress, obj$predicted.oob)
...
writexl::write_xlsx(out, "output/Progression predictors/variable-importance.xlsx")
